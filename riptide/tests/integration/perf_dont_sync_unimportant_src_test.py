...
class PerfDontSyncUnimportantSrcTest(EngineTest):
    def test_dont_sync_unimportant_src(self):
        ...
        for project_ctx in load(
            self, ["integration_all.yml"], [".", "src"], "integration_perf_dont_sync_unimportant_src.yml"
        ):
    ...
    def _common(self, loaded: ProjectLoadResult, files_are_expected_on_host: bool):
        ...
        # host paths
        host_path_synced_simple_with_src = os.path.join(loaded.temp_dir, loaded.src, services["simple_with_src"], "rw")
        ...
        loaded.engine_tester.assert_file_exists(
            PurePosixPath(cnt_path_synced_simple_with_src).joinpath("rw1"),
            loaded.engine,
            project,
            service_objs["simple_with_src"],
        )
        loaded.engine_tester.assert_file_exists(
        ...
        loaded.engine_tester.assert_file_exists(
            PurePosixPath(cnt_path_synced_src_working_directory).joinpath("rw2"),
            loaded.engine,
            project,
            service_objs["src_working_directory"],
        )
        loaded.engine_tester.assert_file_exists(
        ...
        self.assertEqual(
            files_are_expected_on_host,
            os.path.isfile(
                os.path.join(host_path_unimportant_src_working_directory, "unimportant2_added_in_container")
            ),
        )
