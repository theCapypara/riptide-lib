        ...
        for project_ctx in load(self, ["integration_all.yml"], [".", "src"]):
            with project_ctx as loaded:
                ...
                user, group, mode, write_check = loaded.engine_tester.get_permissions_at(
                    ".", loaded.engine, project, project["app"]["services"][service_name], as_user=cpuser.getuid()
                )
    ...
    def test_environment(self):
        for project_ctx in load(self, ["integration_all.yml"], ["."]):
            with project_ctx as loaded:
                ...
                self.assertEqual(
                    "TEST_ENV_VALUE", loaded.engine_tester.get_env("TEST_ENV_KEY", engine, project, service)
                )
                self.assertIsNone(loaded.engine_tester.get_env("TEST_ENV_DOES_NOT_EXIST", engine, project, service))
    ...
    def test_configs(self):
        for project_ctx in load(self, ["integration_all.yml"], ["."]):
            with project_ctx as loaded:
                ...
                self.assertEqual(
                    "no_variable_just_text", loaded.engine_tester.get_file("/config1", loaded.engine, project, service)
                )

                self.assertEqual(
                    project["app"]["name"], loaded.engine_tester.get_file("/config2", loaded.engine, project, service)
                )

                # Assert permissions
                user1, group1, mode1, write_check1 = loaded.engine_tester.get_permissions_at(
                    "/config1", loaded.engine, project, service, is_directory=False, as_user=cpuser.getuid()
                )
                ...
                user2, group2, mode2, write_check2 = loaded.engine_tester.get_permissions_at(
                    "/config2", loaded.engine, project, service, is_directory=False, as_user=cpuser.getuid()
                )
                ...
                self.assertTrue(
                    os.path.isfile(os.path.join(loaded.temp_dir, "_riptide", "processed_config", service_name, "one"))
                )
                self.assertTrue(
                    os.path.isfile(os.path.join(loaded.temp_dir, "_riptide", "processed_config", service_name, "two"))
                )
            ...
            with project_ctx as loaded:
                ...
                for service_name in services:
                    if service_name == "src_working_directory":
                        ...
                        self.assert_response_matches_regex(
                            re.compile(
        ...
        for project_ctx in load(self, ["integration_all.yml"], [".", "src"]):
            with project_ctx as loaded:
                ...
                host_in_volume_path_rw_explicit = os.path.join(
                    loaded.temp_dir, "_riptide", "data", service_name, "in_volume_path_rw_explicit"
                )
                host_in_volume_path_ro = os.path.join(
                    loaded.temp_dir, "_riptide", "data", service_name, "in_volume_path_ro"
                )
                ...
                host_test_auto_create = os.path.join(
                    loaded.temp_dir, "_riptide", "data", service_name, "test_auto_create"
                )
                ...
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_in_volume_path_rw).joinpath("rw2"), loaded.engine, project, service
                )
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_in_volume_path_ro).joinpath("ro1"), loaded.engine, project, service
                )
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_relative_to_project).joinpath("rtp1"), loaded.engine, project, service
                )
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_relative_to_project).joinpath("rtp2"), loaded.engine, project, service
                )
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_relative_to_project).joinpath("rtp3"), loaded.engine, project, service
                )
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_in_volume_path_named).joinpath("named"), loaded.engine, project, service
                )
                ...
                loaded.engine_tester.assert_file_exists(
                    PurePosixPath(cnt_in_volume_path_rw_explicit).joinpath("rw_explicit_added"),
                    loaded.engine,
                    project,
                    service,
                )
                ...
                user1, group1, mode1, write_check = loaded.engine_tester.get_permissions_at(
                    "/in_volume_path_rw", loaded.engine, project, service, as_user=cpuser.getuid()
                )
                ...
                user, group, mode, write_check = loaded.engine_tester.get_permissions_at(
                    "/in_volume_path_ro", loaded.engine, project, service, as_user=cpuser.getuid()
                )
                ...
                self.assertTrue(bool(mode1 & stat.S_IWUSR), "The volume must be writable by group")
                self.assertFalse(
                    write_check, "The volume has to be NOT ACTUALLY writable by user; files must NOT be creatable."
                )
        ...
        for project_ctx in load(self, ["integration_all.yml"], ["."]):
            with project_ctx as loaded:
                ...
                self.assert_response(
                    ...
                    service_root,
                    "/rootcheck",
                    "When running with run_as_current_user == false, a service must with user and group of image",
                )

                # TODO: Group is currently not guaranteed to be the same. Change in the future?
                self.assert_response(
                    str.encode(f"{cpuser.getuid()}\n"),
                    loaded.engine,
                    project,
                    service_no_root,
                    "/rootcheck",
