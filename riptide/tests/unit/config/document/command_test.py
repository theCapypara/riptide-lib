...
class CommandTestCase(unittest.TestCase):
        ...
        self.fix_with_volumes = module.Command.from_dict(
            {
                "additional_volumes": {
                    ...
                    "two": {"host": "./reltest1", "container": "/vol2", "mode": "rw"},
                    "three": {"host": "reltest2", "container": "/vol3", "mode": "rw"},
                    "four": {"host": "reltestc", "container": "reltest_container", "mode": "rw"},
                    "five": {"host": "/absolute_with_ro", "container": "/vol4", "mode": "ro"},
                    "six": {"host": "/absolute_no_mode", "container": "/vol5"},
                },
    ...
    def test_validate_valids(self):
        valid_names = [
            ...
            "valid_via_service.yml",
            "valid_regular_with_volumes_named.yml",
        ]
        for name in valid_names:
            with self.subTest(name=name):
                command = module.Command.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + name))
                self.assertTrue(command.validate())

    def test_validate_invalid_alias_no_aliases(self):
        command = module.Command.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_alias_no_aliases.yml"))
        with self.assertRaises(SchemaError):
            command.validate()

    def test_validate_invalid_regular_no_image(self):
        command = module.Command.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_regular_no_image.yml"))
        with self.assertRaises(SchemaError):
            command.validate()

    def test_validate_invalid_weird_mixup(self):
        command = module.Command.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_weird_mixup.yml"))
        with self.assertRaises(SchemaError):
            command.validate()

    def test_validate_via_service_no_command(self):
        command = module.Command.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_via_service_no_command.yml"))
        with self.assertRaises(SchemaError):
            command.validate()
    ...
    def test_initialize_data_after_variables(self, normalize_mock: Mock):
        ...
        expected = {
            "one": {"host": "NORMALIZED", "container": "/vol1", "mode": "rw"},
            "two": {"host": "NORMALIZED", "container": "/vol2", "mode": "rw"},
            "three": {"host": "NORMALIZED", "container": "/vol3", "mode": "rw"},
            "four": {"host": "NORMALIZED", "container": "reltest_container", "mode": "rw"},
            "five": {"host": "NORMALIZED", "container": "/vol4", "mode": "ro"},
            "six": {"host": "NORMALIZED", "container": "/vol5"},
        }
        cmd.freeze()
    ...
    def test_collect_volumes(
        ...
        serviceRoleA2B1 = {
            ...
            "roles": ["A", "B"],
            "config": {"config2": config2, "config3": config3},
        }
        serviceRoleB2 = {"__UNIT_TEST_NAME": "serviceRoleB2", "roles": ["B"], "config": {"config3": config3}}
        ...
        process_additional_volumes_mock.assert_called_with(
            list(self.fix_with_volumes["additional_volumes"].values()), ProjectStub.FOLDER
        )
    ...
    def test_collect_environment(self, *args, **kwargs):
        cmd = module.Command.from_dict({"environment": {"FROM_ENV": "FROM_ENV"}})
