    ...
    def test_validate_valids(self):
        ...
        for name in valid_names:
            with self.subTest(name=name):
                config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + name))
                self.assertTrue(config.validate())

    def test_validate_invalid_missing_engine(self):
        config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_missing_engine.yml"))
        with self.assertRaisesRegex(SchemaError, "Missing key: 'engine'"):
            config.validate()

    def test_validate_invalid_missing_proxy(self):
        config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_missing_proxy.yml"))
        with self.assertRaisesRegex(SchemaError, "Missing key: 'proxy'"):
            config.validate()

    def test_validate_invalid_missing_repos(self):
        config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_missing_repos.yml"))
        with self.assertRaisesRegex(SchemaError, "Missing key: 'repos'"):
            config.validate()

    def test_validate_invalid_missing_performance(self):
        config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_missing_performance.yml"))
        with self.assertRaisesRegex(SchemaError, "Missing key: 'performance'"):
            config.validate()

    def test_validate_invalid_proxy(self):
        config = module.Config.from_yaml(get_fixture_path(FIXTURE_BASE_PATH + "invalid_proxy.yml"))
